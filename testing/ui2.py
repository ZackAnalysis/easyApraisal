...
def login():
    global matrix
    msg = request.post('http://localhost:5433/login',json=request.form.__dict__())
    return msg
...
def getdata():
    ...
    if job == 'gotoHousePage':
        res = request.post('http://localhost:5433/getdata', json={"job":job,"mslid":mls,"target":target})
        info = res.json()
        matrix['info'].update(info)
